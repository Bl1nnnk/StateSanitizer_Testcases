...
int main()
{
    ...
    DWORD bufferSize = 0;
    DWORD itemCount = 0;
    PPDH_RAW_COUNTER_ITEM_W itemBuffer = nullptr;
    ...
    status = PdhGetRawCounterArrayW(counter, &bufferSize, &itemCount, nullptr);
    ...
    itemBuffer = static_cast<PPDH_RAW_COUNTER_ITEM_W>(malloc(bufferSize));
    if (itemBuffer == nullptr) {
        std::cerr << "Error allocating buffer memory" << std::endl;
        PdhCloseQuery(query);
        return 1;
    }

    // Get the raw counter data
    status = PdhGetRawCounterArrayW(counter, &bufferSize, &itemCount, itemBuffer);
    if (status != ERROR_SUCCESS) {
