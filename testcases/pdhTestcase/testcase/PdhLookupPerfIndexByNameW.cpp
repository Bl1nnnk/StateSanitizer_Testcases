...
int main()
{
    ...
    // Lookup the performance counter index by name
    status = PdhLookupPerfIndexByNameW(nullptr, L"\\Processor(_Total)\\% Processor Time", &index);
    if (status != ERROR_SUCCESS) {
    ...
    status = PdhAddCounterW(query, nullptr, index, &counter);
    if (status != ERROR_SUCCESS) {
