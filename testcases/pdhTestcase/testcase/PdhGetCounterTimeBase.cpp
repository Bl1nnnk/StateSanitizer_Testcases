...
int main()
{
    ...
    // Get the time base for the counter
    status = PdhGetCounterTimeBase(counter, &timeBase);
    if (status != ERROR_SUCCESS) {
        std::cerr << "Error getting counter time base: " << status << std::endl;
        PdhRemoveCounter(counter);
        PdhCloseQuery(query);
        return 1;
    }

    std::cout << "Counter time base: " << timeBase << std::endl;
