...
int main()
{
    ...
    // Add a counter to the query using the index
    ...
    // Look up the index of a performance counter by name
    DWORD index;
    status = PdhLookupPerfIndexByNameA(nullptr, "\\Processor(_Total)\\% Processor Time", &index);
    if (status != ERROR_SUCCESS) {
        std::cerr << "Error looking up performance index: " << status << std::endl;
        if (status == PDH_INVALID_ARGUMENT) std::cout << "111" << std::endl;
        PdhCloseQuery(query);
        return 1;
    }
