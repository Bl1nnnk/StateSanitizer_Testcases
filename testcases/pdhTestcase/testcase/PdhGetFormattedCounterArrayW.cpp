...
int main()
{
    ...
    DWORD bufferSize = 0;
    DWORD itemCount = 0;
    PPDH_FMT_COUNTERVALUE_ITEM_W itemBuffer = nullptr;
    ...
    for (int fmt = PDH_FMT_LONG; fmt <= PDH_FMT_LARGE; fmt *= 2) {
        for (int ret = PDH_FMT_1000; ret <= PDH_FMT_NOSCALE; ret *= 2) {
            ...
            status = PdhGetFormattedCounterArrayW(counter, options, &bufferSize, &itemCount, itemBuffer);
            if (status == PDH_MORE_DATA) {
                itemBuffer = (PPDH_FMT_COUNTERVALUE_ITEM_W)malloc(bufferSize);
