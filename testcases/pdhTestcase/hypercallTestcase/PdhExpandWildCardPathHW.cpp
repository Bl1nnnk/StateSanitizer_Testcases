...
int main()
{
    ...
    PZZWSTR mszExpandedPathList = nullptr;
    ...
    status = PdhExpandWildCardPathHW(hDataSource, szWildCardPath, mszExpandedPathList, &pcchPathListLength, dwFlags);
    if (status == PDH_MORE_DATA) {
        mszExpandedPathList = new WCHAR[pcchPathListLength];
        status = PdhExpandWildCardPathHW(hDataSource, szWildCardPath, mszExpandedPathList, &pcchPathListLength, dwFlags);
    }
    if (status != ERROR_SUCCESS) {
